class CustomerPage {
  ...
  fetchStates() {
    fetch(`${this.server}/states`) //this line sends a get request to all states
    ...
    .then(data => { 
      if (data.length == 0) { //This line makes sure that there is a returned value
        ...
        this.state.states = data; //Fills the states property with the correct data
        this.loadStates();
      }
    })
    .catch(error => {
  ...
  onFindCustomer(event) {
    ...
    if (this.$customerId.value != "") { //Ensure that the customer id has been submitted
      this.state.customerId = this.$customerId.value;
      fetch(`${this.url}/${this.state.customerId}`) //Sends call to api
      .then(response => response.json()) //converts response to json
      .then(data => { 
        if (data.status == 404) { //normal error handling
          alert('That customer does not exist in our database'); 
        }
        else {
          this.state.customer = data; //puts data into a property
          this.fillCustomerFields();
          this.enableButtons("found"); //changes ui to show success
        }
      })
      .catch(error => {
        alert('There was a problem getting customer info!'); 
  ...
  onSaveCustomer(event) {
    ...
    if (this.state.customerId == "") {
      fetch(`${this.url}`, { //sends post request to api
        method: 'POST', 
        body: JSON.stringify({ //converts input to json
          customerId: 0, 
      ...
      .then(data => { 
        ...
        if (data.customerId)
        {
          ...
          this.enableButtons("found"); //Change alert back when done testing
      ...
      .catch(error => {
        alert('There was a problem retrieving customer info!'); 
      });
    }
    // updating
    else {
